...
@Autonomous(name ="12 Auto BLUE")
public class TwelveBlueAuto extends OpMode {
    ...
    public void loop() {
        follower.update();
        if (isShootingFar) {
            shooter.update(shooter.getRegressionVelocity(shooter.getDistance(Constants.farBlueShoot.getX(), Constants.farBlueShoot.getY(), alliance), alliance) * multiplier);
        }
        else {
        ...
        if (time > START_TIME && time < START_TIME+FLYWHEEL_SPINUP_TIME) {
            ...
            PathChain path = new PathChain(
                    new Path(
                            new BezierCurve(
                                    Constants.farBlueShoot,
                                    Actions.blueRow2Control,
                                    Actions.blueRow2Target
                            )
                    ),
                    new Path(
                            new BezierLine(
            ...
            path.firstPath().setLinearHeadingInterpolation(Constants.farBlueShoot.getHeading(), 0);
            path.firstPath().setHeadingInterpolation(HeadingInterpolator.linear(
                    Constants.farBlueShoot.getHeading(),
                    Math.PI,
                    0.5
            ));
            path.lastPath().setLinearHeadingInterpolation(Math.PI, SubsystemsManager.getTargetHeading(55, 80, alliance));
            ...
            path.firstPath()
                    .setHeadingInterpolation(
                            HeadingInterpolator.linear(
                                    Math.PI,
                                    SubsystemsManager.getTargetHeading(20, 80, alliance),
                                    0.3
                            )
                    );
            path.lastPath()
